...
func _on_button_checker_pressed():
	if isSelectedSpriteVisible == true and !Global.selected and Global.blackTurn:
...
#Attack Bottom Right
func _on_texture_button_abr_pressed():
	if AttackBottomRight.is_visible_in_tree() == true:
		Global.attacked(Vector2(position.x + 16, position.y + 16))
		Global.getBlackChecker(position)
		position.x = position.x + 32
		position.y = position.y + 32
